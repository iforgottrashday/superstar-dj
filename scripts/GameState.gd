...
const SAVE_PATH := "user://apex_save.json"
const SAVE_VERSION := 1
const AUTOSAVE_TICK_INTERVAL := 10  # ticks between automatic disk writes
...
func _notification(what: int) -> void:
	# Mobile + desktop: save on app pause (Android backgrounding, iOS lifecycle)
	# and on window close. Survives the OS killing our process when the player
	# tabs away or locks the phone mid-run.
	match what:
...
static func delete_saved_game() -> void:
	if FileAccess.file_exists(SAVE_PATH):
		...
		if dir != null:
			dir.remove("apex_save.json")


func save_to_disk() -> bool:
	if player_faction == "":
		return false  # no run in progress, nothing to save
	var payload: Dictionary = {
		"version": SAVE_VERSION,
		"tick": tick,
		"player_faction": player_faction,
		"treasury": treasury,
		"owned_techs": owned_techs.duplicate(true),
		"temp_modifiers": temp_modifiers.duplicate(true),
		"events_seen": events_seen.duplicate(true),
		"pending_event": pending_event,
		"pre_event_speed": pre_event_speed,
		"ticks_until_next_event": ticks_until_next_event,
		"crusade_threshold": crusade_threshold,
		"crusade_triggered": crusade_triggered,
		"khan_threshold": khan_threshold,
		"khan_triggered": khan_triggered,
		"wolf_howl_ready_at": wolf_howl_ready_at,
		"bear_rage_ready_at": bear_rage_ready_at,
		"lion_defense_ready_at": lion_defense_ready_at,
		"eagle_brood_ready_at": eagle_brood_ready_at,
		"croc_strike_ready_at": croc_strike_ready_at,
		"croc_strike_primed": croc_strike_primed,
		"regions": [],
	}
	for r in regions:
		payload["regions"].append(r.to_dict())
...
func load_from_disk() -> bool:
	...
	if typeof(parsed) != TYPE_DICTIONARY:
		push_warning("Save file malformed; ignoring")
		return false
	var d: Dictionary = parsed
	if int(d.get("version", 0)) != SAVE_VERSION:
		push_warning("Save file version mismatch; ignoring")
		return false
	# Wipe current state first so we don't merge old + saved.
	reset_for_new_run()
	tick = int(d.get("tick", 0))
	player_faction = String(d.get("player_faction", ""))
	treasury = int(d.get("treasury", 0))
	owned_techs = d.get("owned_techs", {})
	temp_modifiers = d.get("temp_modifiers", [])
	events_seen = d.get("events_seen", {})
	pending_event = d.get("pending_event", null)
	pre_event_speed = float(d.get("pre_event_speed", 0.0))
	ticks_until_next_event = int(d.get("ticks_until_next_event", 10))
	crusade_threshold = int(d.get("crusade_threshold", 5))
	crusade_triggered = bool(d.get("crusade_triggered", false))
	khan_threshold = int(d.get("khan_threshold", 8))
	khan_triggered = bool(d.get("khan_triggered", false))
	wolf_howl_ready_at = int(d.get("wolf_howl_ready_at", 0))
	bear_rage_ready_at = int(d.get("bear_rage_ready_at", 0))
	lion_defense_ready_at = int(d.get("lion_defense_ready_at", 0))
	eagle_brood_ready_at = int(d.get("eagle_brood_ready_at", 0))
	croc_strike_ready_at = int(d.get("croc_strike_ready_at", 0))
	croc_strike_primed = bool(d.get("croc_strike_primed", false))
	# Reapply region state on top of the freshly-rebuilt regions list.
	var saved_regions: Array = d.get("regions", [])
	for sr in saved_regions:
		var rid: String = String(sr.get("id", ""))
		if regions_by_id.has(rid):
			regions_by_id[rid].from_dict(sr)
	speed = 0.0  # always resume paused so the player can re-orient
	return true
